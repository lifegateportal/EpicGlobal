...
async function buildCandidate(projectName, repoUrl, candidatePort) {
  ...
  const serveCmd = isEpicodespace
    ? 'pm2 start /usr/bin/bash --name ' + quoteForShell(candidateName) + ' --cwd ' + quoteForShell(candidatePath) + ' -- -c ' + quoteForShell(projectName === 'nexusdirector' ? 'PORT=6105 node_modules/.bin/next start' : 'PORT=6105 node artifacts/epicodespace/serve.mjs')
...
async function promoteCandidate(projectName, candidateName, candidatePath, stablePort) {
  ...
  const promoteCmd = isEpicodespace
    ? 'pm2 start /usr/bin/bash --name ' + quoteForShell(projectName) + ' --cwd ' + quoteForShell(stablePath) + ' -- -c ' + quoteForShell(projectName === 'nexusdirector' ? 'PORT=6105 node_modules/.bin/next start' : 'PORT=6105 node artifacts/epicodespace/serve.mjs')
...
async function executeFirstDeploy(projectName, repoUrl, domain, port) {
  ...
  const firstServeCmd = isEpicodespace
    ? 'pm2 start /usr/bin/bash --name ' + quoteForShell(projectName) + ' --cwd ' + quoteForShell(deployPath) + ' -- -c ' + quoteForShell(projectName === 'nexusdirector' ? 'PORT=6105 node_modules/.bin/next start' : 'PORT=6105 node artifacts/epicodespace/serve.mjs')
